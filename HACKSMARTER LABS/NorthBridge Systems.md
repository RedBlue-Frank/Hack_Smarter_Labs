...
![image.png](NorthBridge%20Systems%20(Hard)/image.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%201.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%202.png)

- We have  `Network shares`  that is worthy being looked at later.

### Testing other services

![image.png](NorthBridge%20Systems%20(Hard)/image%203.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%204.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%205.png)
...
- often used with *Just-In-Time admin* or *Tiering*
...
![image.png](NorthBridge%20Systems%20(Hard)/image%206.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%207.png)
...
- When we tested for other services, we had a `Pwn3d!` on `rdp` and lets straight away authentic
    ...
    ![image.png](NorthBridge%20Systems%20(Hard)/image%208.png)
    
    ```bash
    xfreerdp3 /v:10.1.136.48 /u:_securitytestingsvc /p:'4kCc$A@NZvNAdK@' +clipboard /dynamic-resolution /drive:$(pwd),share
    ```
    
    ![image.png](NorthBridge%20Systems%20(Hard)/image%209.png)
    

![image.png](NorthBridge%20Systems%20(Hard)/image%2010.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2011.png)

- We have what seems to be a password `1rUlHB95TVA2I&BCve`  and we can try to password spry.

![image.png](NorthBridge%20Systems%20(Hard)/image%2012.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2013.png)

- User & System Provisioning Checklist

![image.png](NorthBridge%20Systems%20(Hard)/image%2014.png)
...
- We can test for ADCS vulnerabilities
    ...
    ![image.png](NorthBridge%20Systems%20(Hard)/image%2015.png)
    
    - we did not find any, so this is now out of the way for now.

`Scripts`

![image.png](NorthBridge%20Systems%20(Hard)/image%2016.png)

![image.png](NorthBridge%20Systems%20(Hard)/image%2017.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2018.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2019.png)

![image.png](NorthBridge%20Systems%20(Hard)/image%2020.png)
...
- In `C:\Scripts\Server Build Automation` we gathered some interesting findings
    ...
    - We can test these credentials and also try and perform password spraying
        ...
        ![image.png](NorthBridge%20Systems%20(Hard)/image%2021.png)
        
        - Perfomed password spraying and indeed the creds can authenticate user `_svrautomationsvc`
            - Interestingly, `erhodes` has a `STATUS_ACCOUNT_RESTRICTION` .This can mean that 
            the password *is correct*, but logon is restricted (logon hours, workstation restrictions.
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2022.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2023.png)

- `erhodesT0` is also a member of Domain Admins

![image.png](NorthBridge%20Systems%20(Hard)/image%2024.png)

- `_securitytestingsvc` doesn't have any interesting groups

![image.png](NorthBridge%20Systems%20(Hard)/image%2025.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2026.png)
...
- By default, **any authenticated user** can add **10** machines to AD (ms-DS-MachineAccountQuota = 10). Some environments set it to **0**, meaning *no* normal user can add computers.
- Let me take you back to the findings we made above
    
    ![image.png](NorthBridge%20Systems%20(Hard)/image%2027.png)
    
    - This script confirms exactly *why* the `_svrautomationsvc` account exists, **what OU it can write to.**
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2028.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2029.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2030.png)

- Failed to impersonate with `erhodesT0`.
- We looked into other Admin accounts we can try to impersonate.

![image.png](NorthBridge%20Systems%20(Hard)/image%2031.png)

![image.png](NorthBridge%20Systems%20(Hard)/image%2032.png)

```bash
getST.py -spn "cifs/NORTHJMP01.northbridge.corp" -impersonate RHALLT1 'northbridge.corp/RedBlue2$:RedBlue777' -dc-ip 10.1.11.70
```

![image.png](NorthBridge%20Systems%20(Hard)/image%2033.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2034.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2035.png)

## User flag

![image.png](NorthBridge%20Systems%20(Hard)/image%2036.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2037.png)

![image.png](NorthBridge%20Systems%20(Hard)/image%2038.png)

![image.png](NorthBridge%20Systems%20(Hard)/image%2039.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2040.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2041.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2042.png)
...
- For some reason, the SAM and SECURITY hives were successfully downloaded but not the SYSTEM
    ...
    ![image.png](NorthBridge%20Systems%20(Hard)/image%2043.png)
    ...
    ![image.png](NorthBridge%20Systems%20(Hard)/image%2044.png)
    
    - Retrieved `N**orthdc01$` machine account password**.

![image.png](NorthBridge%20Systems%20(Hard)/image%2045.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2046.png)

```bash
nxc smb 10.1.11.70 -u Administrator -H 'REDACTED'
```

![image.png](NorthBridge%20Systems%20(Hard)/image%2047.png)
...
![image.png](NorthBridge%20Systems%20(Hard)/image%2048.png)

![image.png](NorthBridge%20Systems%20(Hard)/image%2049.png)
